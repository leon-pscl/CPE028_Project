    ...
    <div className={`flex items-center ${isLeft ? 'flex-row' : 'flex-row-reverse'} gap-3`}>
      ...
      <div
        ...
        className={`group relative flex w-full max-w-[220px] cursor-pointer items-start gap-2.5 rounded-lg border p-3 text-left transition-all hover:shadow-md ${
          item.completed ? 'border-brand-300 bg-brand-50' : 'border-gray-200 bg-white hover:border-gray-300'
        }`}
      >
        <button
          ...
          className={`mt-0.5 h-5 w-5 shrink-0 rounded-full border-2 flex items-center justify-center transition-colors cursor-pointer ${
            item.completed ? 'border-brand-500 bg-brand-500 text-white' : 'border-gray-300 group-hover:border-brand-400'
          }`}
...
// ---------------------------------------------------------------------------
function MainNode({
  step, index, total, direction, onToggle,
}: {
  step: RoadmapStep
  index: number
  total: number
      ...
      {hasSubItems && (
        <div className="relative mb-6">
          ...
          <svg className="absolute inset-0 w-full h-full pointer-events-none" style={{ zIndex: 0 }}>
            {leftItems.map((_, i) => (
              <line key={`left-${i}`} x1="50%" y1={`${(i + 0.5) * (100 / maxSubRows)}%`} x2="33%" y2={`${(i + 0.5) * (100 / maxSubRows)}%`} stroke="#d1d5db" strokeWidth="1.5" strokeDasharray="4 3" />
            ))}
            {rightItems.map((_, i) => (
              <line key={`right-${i}`} x1="50%" y1={`${(i + 0.5) * (100 / maxSubRows)}%`} x2="67%" y2={`${(i + 0.5) * (100 / maxSubRows)}%`} stroke="#d1d5db" strokeWidth="1.5" strokeDasharray="4 3" />
            ))}
          </svg>
        </div>
      )}
...
export default function NavigatePage() {
  ...
  const toggleStep = (id: string) => {
    ...
    setSteps(prev => prev.map(s => {
      ...
      if (s.subItems) {
        return { ...s, subItems: s.subItems.map(sub => sub.id === id ? { ...sub, completed: !sub.completed } : sub) }
      }
      return s
    }))
  }
