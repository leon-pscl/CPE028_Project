# Rev.Tech — Frontend Decisions

> **For agents working on the frontend.**
> This document captures the actual frontend implementation decisions — not the ideal/planned architecture.
...
| Google OAuth `signInWithOAuth()` wired | ✅ Done (Iteration 4) |
...
| Role auth review on Connect page | ✅ Done (Iteration 4) |

These are tracked for completion in Iteration 5 (see `docs/project/ROADMAP.md`).
...
- **Note**: Roadmap layout being redesigned in Iteration 4 — horizontal scrollable timeline replaces vertical branching per `rmaptest.html` reference. See ROADMAP.md §4E.
...
- **Role auth review**: Completed per ROADMAP.md §4D.3 — auth gate, RLS, route protection, rate-limit uniformity all verified. See docs/project/ROADMAP.md §4D for findings.
...
└── features/
    ...
    │   ├── RegisterPage.tsx            # Registration + Google OAuth
    │   ├── ProfilePage.tsx             # User info, edit name, sign out, assessment history
    │   ├── ForgotPasswordPage.tsx      # Password reset
    │   └── AuthCallbackPage.tsx        # OAuth/email confirmation callback
    ├── navigate/
    │   ├── NavigatePage.tsx            # Horizontal scrollable timeline with detail panel
    │   └── roadmapData.ts             # REPAIR + RECYCLE step definitions with details
    ├── connect/
...
## 8. Key Deviations from Plan

These are intentional decisions that differ from the ideal plan:

| Planned | Actual Implementation | Reason | Status |
|---|---|---|---|
| Auth uses `public.users` table via trigger | Auth uses `user_metadata` only — no DB query | Avoid 400 errors from missing/misconfigured `users` table | Permanent |
| Google OAuth via `signInWithOAuth()` | Wired in Iteration 4 | Done | ✅ Done |
| Anonymous sessions via `signInAnonymously()` | Not implemented — auth gate modals used instead | Deferred to Iteration 5 | ⏳ |
| `user_transactions` ledger table | Schema exists but transactions not written | Deferred — assessment results not persisted yet | ⏳ |
| Google Places API for dynamic directory | Geoapify Places API used instead | Geoapify offers free tier, no API key restrictions | Permanent |
| Assessment DB persistence via `create_assessment_tx` | Client-side scoring only — no DB writes | Deferred to Iteration 5 | ⏳ |
| Roadmap state persisted to `checklist_completions` | In-memory state only — does not survive reload | Deferred to Iteration 5 | ⏳ |
| Marker clustering (`leaflet.markercluster`) | Library installed but never initialized | Deferred to Iteration 5 | ⏳ |
| Radius slider (1–25 km) for directory search | `searchRadius` hardcoded at 5000m | Deferred to Iteration 5 | ⏳ |
| Continuous geolocation (`watchPosition`) | One-shot `getCurrentPosition` only | Deferred to Iteration 5 | ⏳ |
| Tile fallback layer | Single OSM tile layer — no fallback | Deferred to Iteration 5 | ⏳ |
| `/profile/history` page | Done in Iteration 4 | Shows all past assessments | ✅ Done |
| Serverless Functions (Vercel) for API | No API functions — all logic is client-side | Deferred to Iteration 5 | ⏳ |
