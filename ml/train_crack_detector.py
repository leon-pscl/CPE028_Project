...
Binary classification model to detect cracks in device screens/components.
Uses project_data/train and project_data/test folders.

Dataset structure:
  project_data/
    train/
      cracked/       (cracked screen images)
      not_cracked/   (intact screen images)
    valid/
      cracked/
      not_cracked/
    test/
...
DATASET_ROOT = Path(__file__).parent / "datasets" / "project_data"
MODEL_OUTPUT_PATH = Path(__file__).parent / "models" / "crack_detector.pth"
...
def train_crack_detector():
    """Main training function for crack detection"""
    ...
    # Create model
    model = CrackDetector(num_classes=NUM_CLASSES, pretrained=True)
    ...
    try:
        ...
        if valid_path.exists():
            ...
            print(f"  ⚠ Valid set not found, using train split")
            valid_dataset = None
    ...
    metadata_path = Path(__file__).parent / "models" / "crack_detector_metadata.json"
...
if __name__ == "__main__":
    model = train_crack_detector()
    
    print("\n" + "="*60)
    print("✅ Crack detector training complete!")
    print("="*60)
    print(f"\nUsage:")
    print(f"  from predict_unified import predict_cracks")
    print(f"  result = predict_cracks('path/to/image.jpg')")
    print(f"  # Returns: cracked or not_cracked")
